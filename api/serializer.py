...
class LimitSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        max_days_data = Days.objects.filter(user_id = 1).order_by('consumption').last()
        min_days_data = Days.objects.filter(user_id = 1).order_by('consumption').first()
        max_months_data = Months.objects.filter(user_id = 1).order_by('consumption').last()
        min_months_data = Months.objects.filter(user_id = 1).order_by('consumption').first()
